(function (global) {
  ...
  class Editor {
    ...
    render() {
      ...
      ctx.fillStyle=c.bg;ctx.fillRect(0,0,width,height);this.drawGrid(ctx,width,height,c);this.drawSectionHeader(ctx,"服务","service",c.accent);this.drawSectionHeader(ctx,"命名卷","volume",c.volume);this.drawSectionHeader(ctx,"网络","network",c.network);
    ...
    drawGrid(ctx,width,height,c) { ctx.save();ctx.fillStyle=c.muted;ctx.globalAlpha=.42;for(let y=10;y<height;y+=20)for(let x=10;x<width;x+=20){ctx.beginPath();ctx.arc(x,y,1,0,Math.PI*2);ctx.fill();}ctx.restore(); }
