(function (global) {
  ...
  class Editor {
    ...
    drawOrthogonalMountLinks(ctx,c,links=this.mountLinks()) {
      const lanes=new Map();ctx.save();ctx.strokeStyle=c.muted;ctx.lineWidth=1.5;ctx.setLineDash([6,5]);
      links.forEach(link=>{const from=link.points[0],to=link.points[1],key=Math.round(from.y),lane=lanes.get(key)||0;lanes.set(key,lane+1);const available=Math.max(24,to.y-from.y-18),midY=from.y+Math.min(available,28+lane*10),dx=to.x-from.x;if(Math.abs(dx)<2){ctx.beginPath();ctx.moveTo(from.x,from.y);ctx.lineTo(to.x,to.y);ctx.stroke();return;}const direction=dx>0?1:-1,radius=Math.min(8,Math.abs(dx)/2,Math.max(2,(to.y-midY)/2));ctx.beginPath();ctx.moveTo(from.x,from.y);ctx.lineTo(from.x,midY-radius);ctx.quadraticCurveTo(from.x,midY,from.x+direction*radius,midY);ctx.lineTo(to.x-direction*radius,midY);ctx.quadraticCurveTo(to.x,midY,to.x,midY+radius);ctx.lineTo(to.x,to.y);ctx.stroke();});ctx.restore();
    }
