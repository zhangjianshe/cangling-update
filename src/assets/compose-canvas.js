(function (global) {
  ...
  class Editor {
    ...
    drawExportMountLinks(ctx,c) {
      const links=this.mountLinks(),lanes=new Map();ctx.save();ctx.strokeStyle=c.muted;ctx.lineWidth=1.5;ctx.setLineDash([6,5]);
    ...
    exportPng(title,filename) {
      ...
      try {
        ...
        ctx.fillStyle=c.bg;ctx.fillRect(0,0,logicalWidth,logicalHeight);ctx.fillStyle=c.text;ctx.font='700 22px "Noto Sans CJK SC","Microsoft YaHei","PingFang SC",system-ui,sans-serif';ctx.textBaseline="middle";ctx.fillText(titleText,margin,titleHeight/2+10);ctx.translate(margin-minX,titleHeight+margin-minY);this.drawExportMountLinks(ctx,c);this.drawLinks(ctx,c);this.model.volumes.forEach((value,index)=>this.drawVolume(ctx,value,index,c));this.model.networks.forEach((value,index)=>this.drawNetwork(ctx,value,index,c));this.model.services.forEach(service=>this.drawService(ctx,service,c));ctx.fillStyle=c.accent;ctx.font="700 12px system-ui";ctx.fillText("服务",24,18);ctx.fillStyle=c.volume;ctx.fillText("数据卷",24,this.volumeTop()-12);ctx.fillStyle=c.network;ctx.fillText("网络",24,this.networkTop()-12);
