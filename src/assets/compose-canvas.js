(function (global) {
  ...
  class Editor {
    ...
    exportPng(title,filename) { const saved={selected:this.selected,selectedVolume:this.selectedVolume,selectedNetwork:this.selectedNetwork,selectedLink:this.selectedLink,linkFrom:this.linkFrom,linkTarget:this.linkTarget,envFileTarget:this.envFileTarget,resourceTarget:this.resourceTarget,volumePanelService:this.volumePanelService,portPanelService:this.portPanelService,networkPanelService:this.networkPanelService,environmentPanelService:this.environmentPanelService,envFileMenuService:this.envFileMenuService,drag:this.drag};Object.assign(this,{selected:"",selectedVolume:"",selectedNetwork:"",selectedLink:null,linkFrom:"",linkTarget:"",envFileTarget:"",resourceTarget:"",volumePanelService:"",portPanelService:"",networkPanelService:"",environmentPanelService:"",envFileMenuService:"",drag:null});try{const boxes=[...this.model.services.map(service=>this.serviceBox(service)),...this.model.volumes.map((_,index)=>this.volumeBox(index)),...this.model.networks.map((_,index)=>this.networkBox(index))],minX=Math.min(0,...boxes.map(box=>box.x)),minY=0,maxX=Math.max(180,...boxes.map(box=>box.x+box.w)),maxY=Math.max(100,...boxes.map(box=>box.y+box.h)),margin=48,titleHeight=54,logicalWidth=Math.ceil(maxX-minX+margin*2),logicalHeight=Math.ceil(maxY-minY+margin*2+titleHeight),scale=Math.min(2,8192/logicalWidth,8192/logicalHeight),canvas=document.createElement("canvas");canvas.width=Math.max(1,Math.floor(logicalWidth*scale));canvas.height=Math.max(1,Math.floor(logicalHeight*scale));const ctx=canvas.getContext("2d");ctx.scale(scale,scale);const c={bg:color("--bg-2","#161b22"),card:color("--bg-3","#21262d"),text:color("--text","#f0f6fc"),muted:color("--muted","#8b949e"),line:color("--line","#30363d"),accent:color("--accent","#58a6ff"),active:color("--bg-active","#1f6feb33"),volume:"#9a6700",volumeActive:"rgba(154,103,0,.16)",port:"#1a7f37",portActive:"rgba(26,127,55,.16)",network:"#8250df",networkActive:"rgba(130,80,223,.16)",environment:"#bc4c00",environmentActive:"rgba(188,76,0,.16)"};ctx.fillStyle=c.bg;ctx.fillRect(0,0,logicalWidth,logicalHeight);ctx.fillStyle=c.text;ctx.font="700 22px system-ui";ctx.textBaseline="middle";ctx.fillText(String(title||"Compose 服务拓扑"),margin,titleHeight/2+10);ctx.translate(margin-minX,titleHeight+margin-minY);ctx.save();ctx.strokeStyle=c.muted;ctx.lineWidth=1.5;ctx.setLineDash([6,5]);[...this.mountLinks(),...this.networkLinks()].forEach(link=>{ctx.beginPath();ctx.moveTo(link.points[0].x,link.points[0].y);ctx.lineTo(link.points[1].x,link.points[1].y);ctx.stroke();});ctx.restore();this.drawLinks(ctx,c);this.model.volumes.forEach((value,index)=>this.drawVolume(ctx,value,index,c));this.model.networks.forEach((value,index)=>this.drawNetwork(ctx,value,index,c));this.model.services.forEach(service=>this.drawService(ctx,service,c));ctx.fillStyle=c.accent;ctx.font="600 12px system-ui";ctx.fillText("服务",24,18);ctx.fillStyle=c.volume;ctx.fillText("命名卷",24,this.volumeTop()-12);ctx.fillStyle=c.network;ctx.fillText("网络",24,this.networkTop()-12);canvas.toBlob(blob=>{if(!blob){this.onStatus("PNG 导出失败");return;}const url=URL.createObjectURL(blob),anchor=document.createElement("a");anchor.href=url;anchor.download=String(filename||"compose-graph.png");document.body.appendChild(anchor);anchor.click();anchor.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);this.onStatus("Graph PNG 已导出");},"image/png");}catch(error){this.onStatus("PNG 导出失败："+error.message);}finally{Object.assign(this,saved);this.render();} }
    drawMounts(ctx,c) {
