(function (global) {
  ...
  class Editor {
    constructor(options) {
      Object.assign(this, { selected: "", selectedVolume: "", selectedNetwork: "", selectedLink: null, volumePanelService: "", portPanelService: "", networkPanelService: "", hoverLink: null, hoverServiceVolumeRow: null, hoverServicePortRow: null, hoverServiceNetworkRow: null, linkFrom: "", linkTarget: "", drag: null, pointer: { x: 0, y: 0 } }, options);
    ...
    networkPanelWidth(service) { const ctx=this.canvas.getContext("2d"),handle=this.networkHandleBox(service),x=handle.x+handle.w/2,canvasWidth=parseFloat(this.canvas.style.width||"820");ctx.save();ctx.font="600 12px ui-monospace, monospace";const textWidth=(service.networks||[]).reduce((width,name)=>Math.max(width,ctx.measureText(name).width),0);ctx.restore();return Math.min(Math.max(150,Math.ceil(textWidth)+66),Math.max(150,canvasWidth-x)); }
    ...
    render() {
      ...
      ctx.fillStyle=c.bg;ctx.fillRect(0,0,width,height);this.drawGrid(ctx,width,height,c);this.drawSectionHeader(ctx,"命名卷","volume",c.volume);this.drawSectionHeader(ctx,"网络","network",c.network);
      ...
      this.drawServiceVolumes(ctx,c);this.drawServicePorts(ctx,c);ctx.save();ctx.translate(0,5);this.drawServiceNetworks(ctx,c);ctx.restore();this.drawPanelChrome(ctx,"volume",c);this.drawPanelChrome(ctx,"port",c);this.drawPanelChrome(ctx,"network",c);this.drawPanelAddIcon(ctx,"volume",c);this.drawPanelAddIcon(ctx,"port",c);this.drawPanelAddIcon(ctx,"network",c);
    }
    drawMounts(ctx,c) {
    ...
    drawGrid(ctx,width,height,c) { ctx.save();ctx.fillStyle=c.line;ctx.globalAlpha=.42;for(let y=10;y<height;y+=20)for(let x=10;x<width;x+=20){ctx.beginPath();ctx.arc(x,y,.7,0,Math.PI*2);ctx.fill();}ctx.restore(); }
