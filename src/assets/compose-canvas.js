(function (global) {
  ...
  class Editor {
    ...
    serviceNetworkDeleteAt(service,index) { const h=this.networkHandleBox(service);return{x:h.x+h.w/2+20,y:h.y+h.h+44+index*26}; }
    serviceNetworkAddAt(service) { const h=this.networkHandleBox(service),x=h.x+h.w/2,width=this.networkPanelWidth(service);return{x:x-14+width-20,y:h.y+h.h+18}; }
    ...
    render() {
      ...
      this.drawServiceVolumes(ctx,c);this.drawServicePorts(ctx,c);this.drawServiceNetworks(ctx,c);
    }
    drawMounts(ctx,c) {
