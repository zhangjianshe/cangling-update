(function (global) {
  ...
  class Editor {
    constructor(options) {
      Object.assign(this, { selected: "", selectedVolume: "", selectedNetwork: "", selectedLink: null, volumePanelService: "", portPanelService: "", networkPanelService: "", hoverLink: null, hoverServiceVolumeRow: null, hoverServicePortRow: null, hoverServiceNetworkRow: null, linkFrom: "", linkTarget: "", drag: null, pointer: { x: 0, y: 0 }, panX: 0, panY: 0 }, options);
    ...
    volumeTop() { let bottom=80;this.model.services.forEach(service=>{const b=this.serviceBox(service);bottom=Math.max(bottom,b.y+b.h);});const volumeService=this.model.services.find(service=>service.name===this.volumePanelService),portService=this.model.services.find(service=>service.name===this.portPanelService),networkService=this.model.services.find(service=>service.name===this.networkPanelService),panelService=volumeService||portService||networkService,items=volumeService?this.serviceVolumes(volumeService):(portService?portService.ports:(networkService?networkService.networks:[]));if(panelService){const handle=volumeService?this.mountHandleBox(panelService):(portService?this.portHandleBox(panelService):this.networkHandleBox(panelService));bottom=Math.max(bottom,handle.y+handle.h+18+(items.length+1)*26);}return bottom+72; }
    ...
    point(e) { const r = this.canvas.getBoundingClientRect(); return { x: e.clientX - r.left - this.panX, y: e.clientY - r.top - this.panY }; }
    hitService(p) {
    ...
    pointerDown(e) {
      ...
      if(mountHandle){this.selected="";this.selectedVolume="";this.selectedNetwork="";this.selectedLink=null;this.volumePanelService=mountHandle.name;this.portPanelService="";this.networkPanelService="";this.drag=null;this.canvas.style.cursor="pointer";this.canvas.focus();this.renderInspector();this.onStatus(`已打开 ${mountHandle.name} 的数据卷`);this.render();return;}
      if(portHandle){this.selected="";this.selectedVolume="";this.selectedNetwork="";this.selectedLink=null;this.volumePanelService="";this.portPanelService=portHandle.name;this.networkPanelService="";this.drag=null;this.canvas.style.cursor="pointer";this.canvas.focus();this.renderInspector();this.onStatus(`已打开 ${portHandle.name} 的端口`);this.render();return;}
      if(networkHandle){this.selected="";this.selectedVolume="";this.selectedNetwork="";this.selectedLink=null;this.volumePanelService="";this.portPanelService="";this.networkPanelService=networkHandle.name;this.drag=null;this.canvas.style.cursor="pointer";this.canvas.focus();this.renderInspector();this.onStatus(`已打开 ${networkHandle.name} 的网络`);this.render();return;}
      ...
      if (service) {
        this.selected = service.name; this.selectedVolume = "";this.selectedNetwork=""; this.selectedLink = null; this.volumePanelService="";this.portPanelService="";this.networkPanelService=""; const pos = this.positions[service.name];
    ...
    keyDown(e) {
      ...
      else if (e.key === "Escape" && (this.selectedLink || this.selectedVolume || this.selectedNetwork || this.volumePanelService || this.portPanelService || this.networkPanelService)) { e.preventDefault(); this.selectedLink = null; this.selectedVolume = "";this.selectedNetwork=""; this.volumePanelService="";this.portPanelService="";this.networkPanelService=""; this.onStatus("已取消选择"); this.renderInspector(); this.render(); }
    }
    ...
    addService() { let index=1,name="service-1";while(this.model.services.some(service=>service.name===name))name="service-"+(++index);this.yaml=addServiceYaml(this.yaml,name);this.model=parse(this.yaml);this.ensurePositions();this.selected=name;this.selectedVolume="";this.selectedNetwork="";this.volumePanelService="";this.portPanelService="";this.networkPanelService="";this.onChange(this.yaml,`已添加服务 ${name}`);this.onStatus(`已添加服务 ${name}`);this.renderInspector();this.render(); }
    addVolume() {
    ...
    renderInspector() {
      ...
      this.inspector.innerHTML = `<div class="compose-inspector-title">${html(service.name)}</div>${field("镜像","image",service.image)}${field("容器名称","containerName",service.containerName)}${field("启动命令","command",service.command)}${restartField}<button type="button" class="btn primary" data-apply>应用到草稿</button>`;
    ...
    render() {
      ...
      ctx.fillStyle=c.bg;ctx.fillRect(0,0,width,height);this.drawGrid(ctx,width,height,c);ctx.translate(this.panX,this.panY);this.drawSectionHeader(ctx,"服务","service",c.accent);this.drawSectionHeader(ctx,"命名卷","volume",c.volume);this.drawSectionHeader(ctx,"网络","network",c.network);
      ...
      this.drawUnifiedServicePanel(ctx,c);this.drawServiceNetworkIps(ctx,c);
    }
    drawMounts(ctx,c) {
    ...
    drawGrid(ctx,width,height,c) { const ox=((this.panX+10)%20+20)%20,oy=((this.panY+10)%20+20)%20;ctx.save();ctx.fillStyle=c.muted;ctx.globalAlpha=.42;for(let y=oy;y<height;y+=20)for(let x=ox;x<width;x+=20){ctx.beginPath();ctx.arc(x,y,1,0,Math.PI*2);ctx.fill();}ctx.restore(); }
    ...
    drawService(ctx,s,c) {
      const b=this.serviceBox(s),isTarget=s.name===this.linkTarget,linked=s.volumes.some(mount=>this.model.volumes.includes(mount.split(":")[0])),panelOpen=s.name===this.volumePanelService,portsOpen=s.name===this.portPanelService,volumeTone=s.volumes.length?c.volume:c.muted,portTone=s.ports.length?c.port:c.muted,networkTone=s.networks.length?c.network:c.muted;ctx.save();if(isTarget){ctx.shadowColor=c.accent;ctx.shadowBlur=12;}roundRect(ctx,b.x,b.y,b.w,b.h,6);ctx.fillStyle=(s.name===this.selected||isTarget)?c.active:c.card;ctx.fill();ctx.strokeStyle=(s.name===this.selected||s.name===this.linkFrom||isTarget||linked)?c.accent:c.line;ctx.lineWidth=isTarget?3:((s.name===this.selected||s.name===this.linkFrom)?2:1);ctx.stroke();ctx.restore();ctx.fillStyle=c.text;ctx.font="600 14px system-ui";ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText(clip(ctx,s.name,b.w-24),b.x+b.w/2,b.y+(b.h-8)/2);ctx.textAlign="left";ctx.textBaseline="alphabetic";
      ctx.fillStyle=c.bg;ctx.fillRect(b.x+b.w/2-54,b.y+b.h-1,b.w>108?108:b.w,2);
      ...
      const nh=this.networkHandleBox(s),nx=nh.x+nh.w/2,ny=nh.y+nh.h/2,networksOpen=s.name===this.networkPanelService;roundRect(ctx,nh.x,nh.y,nh.w,nh.h,3);ctx.fillStyle=networksOpen&&s.networks.length?c.networkActive:c.card;ctx.fill();ctx.strokeStyle=networkTone;ctx.lineWidth=networksOpen?2:1.5;ctx.stroke();ctx.beginPath();ctx.arc(nx,ny,5,0,Math.PI*2);ctx.stroke();ctx.beginPath();ctx.arc(nx,ny,1.6,0,Math.PI*2);ctx.fillStyle=networkTone;ctx.fill();
    }
