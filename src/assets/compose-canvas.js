(function (global) {
  ...
  class Editor {
    constructor(options) {
      Object.assign(this, { selected: "", selectedVolume: "", selectedNetwork: "", selectedLink: null, envFileSelected: false, volumePanelService: "", portPanelService: "", networkPanelService: "", environmentPanelService: "", hoverLink: null, hoverServiceVolumeRow: null, hoverServicePortRow: null, hoverServiceNetworkRow: null, hoverServiceEnvironmentRow: null, linkFrom: "", linkTarget: "", drag: null, pointer: { x: 0, y: 0 }, panX: 0, panY: 0, zoom: 1 }, options);
    ...
    pointerDown(e) {
      ...
      if(this.hitEnvFile(p)){this.selected="";this.selectedVolume="";this.selectedNetwork="";this.selectedLink=null;this.envFileSelected=true;this.volumePanelService="";this.portPanelService="";this.networkPanelService="";this.environmentPanelService="";this.canvas.focus();if(typeof this.onOpenEnvFile==="function")this.onOpenEnvFile();else this.onStatus(".env 编辑器不可用");this.render();return;}this.envFileSelected=false;
    ...
    render() {
      ...
      this.drawUnifiedServicePanel(ctx,c);this.drawEnvironmentPanel(ctx,c);this.drawServiceNetworkIps(ctx,c);
    }
    drawMounts(ctx,c) {
    ...
    drawService(ctx,s,c) {
      const b=this.serviceBox(s),isTarget=s.name===this.linkTarget,linked=s.volumes.some(mount=>this.model.volumes.includes(mount.split(":")[0])),panelOpen=s.name===this.volumePanelService,portsOpen=s.name===this.portPanelService,volumeTone=s.volumes.length?c.volume:c.muted,portTone=s.ports.length?c.port:c.muted,networkTone=s.networks.length?c.network:c.muted,environmentTone=s.environment.length?c.environment:c.muted;ctx.save();if(isTarget){ctx.shadowColor=c.accent;ctx.shadowBlur=12;}roundRect(ctx,b.x,b.y,b.w,b.h,6);ctx.fillStyle=(s.name===this.selected||isTarget)?c.active:c.card;ctx.fill();ctx.strokeStyle=(s.name===this.selected||s.name===this.linkFrom||isTarget||linked)?c.accent:c.line;ctx.lineWidth=isTarget?3:((s.name===this.selected||s.name===this.linkFrom)?2:1);ctx.stroke();ctx.restore();ctx.fillStyle=c.text;ctx.font="600 14px system-ui";ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText(clip(ctx,s.name,b.w-24),b.x+b.w/2,b.y+(b.h-8)/2);ctx.textAlign="left";ctx.textBaseline="alphabetic";
