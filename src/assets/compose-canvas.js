(function (global) {
  ...
  function parse(yaml) {
    ...
    for (const raw of String(yaml || "").split(/\r?\n/)) {
      ...
      if (section === "services" && n === 2 && /^[^:]+:\s*$/.test(text)) {
        service = { name: clean(text.slice(0, -1)), title: "", image: "", containerName: "", command: "", restart: "", user: "", healthcheck: null, depends: [], networks: [], networkIps: {}, volumes: [], ports: [], environment: [], envFiles: [], labels: [] };
      ...
      if (section === "services" && service) {
        if (n === 4) {
          ...
          else if (key === "labels") service.labels.push(...inlineList(value));
        ...
        if (n >= 6 && text.startsWith("- ")) {
          ...
          else if (nested === "labels") service.labels.push(value);
        ...
        } else if(n===6&&(nested==="environment"||nested==="labels")) { const at=text.indexOf(":");if(at>0){const key=clean(text.slice(0,at)),value=clean(text.slice(at+1));service[nested].push(key+(value?"="+value:""));}
    ...
    for (const item of model.services) {
      ...
      item.labels = unique(item.labels);
      const titleLabel=item.labels.find(value=>value.split("=",1)[0].trim()==="title");
      item.title=titleLabel?titleLabel.slice(titleLabel.indexOf("=")+1).trim():item.name;
    }
  ...
  function updateServiceYaml(yaml, name, values) {
    ...
    const fields = [
      ...
      ["volumes", "volumes", true], ["networks", "networks", true], ["environment", "environment", true], ["labels", "labels", true], ["env_file", "envFiles", true],
    ];
  ...
  class Editor {
    ...
    pointerDown(e) {
      ...
      if (service) {
        ...
        this.drag={type:"resource",kind:"volume",name:volume,x:e.clientX,y:e.clientY,moved:false};this.resourceTarget="";this.canvas.style.cursor="grabbing";this.canvas.setPointerCapture(e.pointerId);e.preventDefault();
      } else if(network){this.drag={type:"resource",kind:"network",name:network,x:e.clientX,y:e.clientY,moved:false};this.resourceTarget="";this.canvas.style.cursor="grabbing";this.canvas.setPointerCapture(e.pointerId);e.preventDefault();
      } else {
    ...
    pointerMove(e) {
      ...
      if(this.drag.type==="resource"){this.drag.moved=this.drag.moved||Math.hypot(e.clientX-this.drag.x,e.clientY-this.drag.y)>4;const target=this.hitService(p);this.resourceTarget=target?target.name:"";this.canvas.style.cursor="grabbing";this.render();return;}
    ...
    pointerUp(e) {
      ...
      if(this.drag.type==="resource"){const drag=this.drag,moved=drag.moved,target=e.type==="pointercancel"?null:(this.model.services.find(service=>service.name===this.resourceTarget)||this.hitService(p));this.drag=null;this.resourceTarget="";this.canvas.style.cursor="default";if(moved&&target){this.selected=target.name;this.selectedVolume="";this.selectedNetwork="";if(drag.kind==="network"){if(target.networks.includes(drag.name)){this.onStatus(`${target.name} 已关联网络 ${drag.name}`);this.render();return;}this.commit(target,{networks:[...target.networks,drag.name],networkIps:{...target.networkIps}},`已为 ${target.name} 添加网络 ${drag.name}`);return;}if(target.volumes.some(mount=>mount.split(":")[0]===drag.name)){this.onStatus(`${target.name} 已挂载命名卷 ${drag.name}`);this.render();return;}const mount=`${drag.name}:/${drag.name}`;this.commit(target,{volumes:[...target.volumes,mount]},`已为 ${target.name} 添加挂载 ${mount}`);return;}if(!moved&&e.type!=="pointercancel"){this.selected="";this.selectedLink=null;this.envFileSelected=false;this.volumePanelService="";this.portPanelService="";this.networkPanelService="";this.environmentPanelService="";if(drag.kind==="volume"){this.selectedVolume=drag.name;this.selectedNetwork="";}else{this.selectedVolume="";this.selectedNetwork=drag.name;}this.canvas.focus();this.renderInspector();}this.render();return;}
      if (this.drag.type === "link") {
    ...
    renderInspector() {
      ...
      if (!service) { const note=readHeaderNote(this.yaml);this.inspector.innerHTML=`<div class="compose-inspector-title">Compose</div><label>文件头注释<textarea data-compose-note rows="5" placeholder="输入 Compose 文件说明">${html(note)}</textarea></label><div class="compose-inspector-actions"><button type="button" class="btn primary" data-compose-save>保存 Compose</button><button type="button" class="btn" data-compose-export>导出 PNG</button></div>`;this.inspector.querySelector("[data-compose-save]").onclick=()=>{const yaml=writeHeaderNote(this.yaml,this.inspector.querySelector("[data-compose-note]").value);if(yaml!==this.yaml){this.yaml=yaml;this.model=parse(yaml);this.onChange(yaml,"已更新 Compose 文件头注释");}if(typeof this.onSaveCompose==="function")this.onSaveCompose();};this.inspector.querySelector("[data-compose-export]").onclick=()=>this.exportPng(this.exportTitle||"Compose 服务拓扑",this.exportFilename||"compose-graph.png");return; }
      ...
      this.inspector.querySelector("[data-apply]").onclick = () => {
        ...
        this.inspector.querySelectorAll("[data-field]").forEach(input => {
          ...
          if(key==="title"){const hasTitle=service.labels.some(label=>label.split("=",1)[0].trim()==="title"),nameInput=this.inspector.querySelector('[data-field="name"]'),nextName=nameInput&&nameInput.value.trim()||service.name,title=!hasTitle&&value===service.name&&nextName!==service.name?nextName:(value||nextName);if(!hasTitle||title!==service.title)patch.labels=[...service.labels.filter(label=>label.split("=",1)[0].trim()!=="title"),`title=${title}`];}
          else if (JSON.stringify(value) !== JSON.stringify(service[key])) patch[key] = value;
        });
    ...
    render() {
      ...
      ctx.fillStyle=c.bg;ctx.fillRect(0,0,width,height);this.drawGrid(ctx,width,height,c);ctx.translate(this.panX,this.panY);ctx.scale(this.zoom,this.zoom);this.drawSectionHeader(ctx,"服务","service",c.accent);this.drawEnvFileButton(ctx,c);this.drawSectionHeader(ctx,"数据卷","volume",c.volume);this.drawSectionHeader(ctx,"网络","network",c.network);
      this.drawMounts(ctx,c);this.drawNetworkLinks(ctx,c);this.drawLinks(ctx,c);this.drawEnvFileDrag(ctx,c);this.drawResourceDrag(ctx,c);this.model.volumes.forEach((v,i)=>this.drawVolume(ctx,v,i,c));this.model.networks.forEach((v,i)=>this.drawNetwork(ctx,v,i,c));this.model.services.forEach(s=>this.drawService(ctx,s,c));
    ...
    drawOrthogonalMountLinks(ctx,c,links=this.mountLinks()) {
      const groups=new Map();links.forEach(link=>{const key=link.service||`${link.points[0].x}:${link.points[0].y}`;if(!groups.has(key))groups.set(key,[]);groups.get(key).push(link);});ctx.save();ctx.strokeStyle=c.muted;ctx.fillStyle=c.muted;ctx.lineWidth=1.5;ctx.setLineDash([6,5]);
      groups.forEach(group=>{const from=group[0].points[0],nearestTarget=Math.min(...group.map(link=>link.points[1].y)),junctionY=Math.max(from.y+12,Math.min(from.y+28,nearestTarget-12));ctx.beginPath();ctx.moveTo(from.x,from.y);ctx.lineTo(from.x,junctionY);ctx.stroke();group.forEach(link=>{const to=link.points[1],dx=to.x-from.x;if(Math.abs(dx)<2){ctx.beginPath();ctx.moveTo(from.x,junctionY);ctx.lineTo(to.x,to.y);ctx.stroke();return;}const direction=dx>0?1:-1,radius=Math.min(8,Math.abs(dx)/2,Math.max(2,(to.y-junctionY)/2));ctx.beginPath();ctx.moveTo(from.x,junctionY);ctx.lineTo(to.x-direction*radius,junctionY);ctx.quadraticCurveTo(to.x,junctionY,to.x,junctionY+radius);ctx.lineTo(to.x,to.y);ctx.stroke();});ctx.save();ctx.setLineDash([]);ctx.beginPath();ctx.arc(from.x,junctionY,3,0,Math.PI*2);ctx.fill();ctx.restore();});ctx.restore();
    }
    exportPng(title,filename) {
      const saved={selected:this.selected,selectedVolume:this.selectedVolume,selectedNetwork:this.selectedNetwork,selectedLink:this.selectedLink,linkFrom:this.linkFrom,linkTarget:this.linkTarget,envFileTarget:this.envFileTarget,resourceTarget:this.resourceTarget,volumePanelService:this.volumePanelService,portPanelService:this.portPanelService,networkPanelService:this.networkPanelService,environmentPanelService:this.environmentPanelService,envFileMenuService:this.envFileMenuService,drag:this.drag};
      Object.assign(this,{selected:"",selectedVolume:"",selectedNetwork:"",selectedLink:null,linkFrom:"",linkTarget:"",envFileTarget:"",resourceTarget:"",volumePanelService:"",portPanelService:"",networkPanelService:"",environmentPanelService:"",envFileMenuService:"",drag:null});
      try {
        const boxes=[...this.model.services.map(service=>this.serviceBox(service)),...this.model.volumes.map((_,index)=>this.volumeBox(index)),...this.model.networks.map((_,index)=>this.networkBox(index))],minX=Math.min(0,...boxes.map(box=>box.x)),minY=0,maxX=Math.max(180,...boxes.map(box=>box.x+box.w)),maxY=Math.max(100,...boxes.map(box=>box.y+box.h)),margin=48,titleHeight=54,titleText=String(title||"Compose 服务拓扑"),titleWidth=[...titleText].reduce((width,char)=>width+(/[^\u0000-\u00ff]/.test(char)?23:13),0),logicalWidth=Math.ceil(Math.max(maxX-minX,titleWidth)+margin*2),logicalHeight=Math.ceil(maxY-minY+margin*2+titleHeight),scale=Math.min(2,8192/logicalWidth,8192/logicalHeight),canvas=document.createElement("canvas");
        canvas.width=Math.max(1,Math.floor(logicalWidth*scale));canvas.height=Math.max(1,Math.floor(logicalHeight*scale));const ctx=canvas.getContext("2d");ctx.scale(scale,scale);const c={bg:color("--bg-2","#161b22"),card:color("--bg-3","#21262d"),text:color("--text","#f0f6fc"),muted:color("--muted","#8b949e"),line:color("--line","#30363d"),accent:color("--accent","#58a6ff"),active:color("--bg-active","#1f6feb33"),volume:"#9a6700",volumeActive:"rgba(154,103,0,.16)",port:"#1a7f37",portActive:"rgba(26,127,55,.16)",network:"#8250df",networkActive:"rgba(130,80,223,.16)",environment:"#bc4c00",environmentActive:"rgba(188,76,0,.16)"};
        ctx.fillStyle=c.bg;ctx.fillRect(0,0,logicalWidth,logicalHeight);ctx.fillStyle=c.text;ctx.font='700 22px "Noto Sans CJK SC","Microsoft YaHei","PingFang SC",system-ui,sans-serif';ctx.textBaseline="middle";ctx.fillText(titleText,margin,titleHeight/2+10);ctx.translate(margin-minX,titleHeight+margin-minY);this.drawOrthogonalMountLinks(ctx,c);this.drawLinks(ctx,c);this.model.volumes.forEach((value,index)=>this.drawVolume(ctx,value,index,c));this.model.networks.forEach((value,index)=>this.drawNetwork(ctx,value,index,c));this.model.services.forEach(service=>this.drawService(ctx,service,c));ctx.fillStyle=c.accent;ctx.font="700 12px system-ui";ctx.fillText("服务",24,18);ctx.fillStyle=c.volume;ctx.fillText("数据卷",24,this.volumeTop()-12);ctx.fillStyle=c.network;ctx.fillText("网络",24,this.networkTop()-12);
        canvas.toBlob(blob=>{if(!blob){this.onStatus("PNG 导出失败");return;}const url=URL.createObjectURL(blob),anchor=document.createElement("a");anchor.href=url;anchor.download=String(filename||"compose-graph.png");document.body.appendChild(anchor);anchor.click();anchor.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);this.onStatus("Graph PNG 已导出");},"image/png");
      } catch(error) { this.onStatus("PNG 导出失败："+error.message); }
    ...
    drawResourceDrag(ctx,c) { if(!this.drag||this.drag.type!=="resource")return;const index=(this.drag.kind==="volume"?this.model.volumes:this.model.networks).indexOf(this.drag.name);if(index<0)return;const b=this.drag.kind==="volume"?this.volumeBox(index):this.networkBox(index),tone=this.drag.kind==="volume"?c.volume:c.network;ctx.save();ctx.strokeStyle=tone;ctx.lineWidth=1.5;ctx.setLineDash([5,4]);ctx.beginPath();ctx.moveTo(b.x+b.w/2,b.y+b.h/2);ctx.lineTo(this.pointer.x,this.pointer.y);ctx.stroke();ctx.restore(); }
    ...
    drawService(ctx,s,c) {
      const b=this.serviceBox(s),isLinkTarget=s.name===this.linkTarget,isEnvTarget=s.name===this.envFileTarget,isResourceTarget=s.name===this.resourceTarget,isTarget=isLinkTarget||isEnvTarget||isResourceTarget,linked=s.volumes.some(mount=>this.model.volumes.includes(mount.split(":")[0])),panelOpen=s.name===this.volumePanelService,portsOpen=s.name===this.portPanelService,volumeTone=s.volumes.length?c.volume:c.muted,portTone=s.ports.length?c.port:c.muted,networkTone=s.networks.length?c.network:c.muted,environmentTone=s.environment.length?c.environment:c.muted,targetTone=isEnvTarget?"#0a7f83":(isResourceTarget&&this.drag&&this.drag.kind==="volume"?c.volume:(isResourceTarget?c.network:c.accent));ctx.save();if(isTarget){ctx.shadowColor=targetTone;ctx.shadowBlur=12;}roundRect(ctx,b.x,b.y,b.w,b.h,6);ctx.fillStyle=(s.name===this.selected||isTarget)?c.active:c.card;ctx.fill();ctx.strokeStyle=isTarget?targetTone:((s.name===this.selected||s.name===this.linkFrom||linked)?c.accent:c.line);ctx.lineWidth=isTarget?3:((s.name===this.selected||s.name===this.linkFrom)?2:1);ctx.stroke();ctx.restore();ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillStyle=c.text;ctx.font="600 14px system-ui";ctx.fillText(clip(ctx,s.title||s.name,b.w-48),b.x+b.w/2,b.y+23);ctx.fillStyle=c.muted;ctx.font="400 11px ui-monospace, monospace";ctx.fillText(clip(ctx,s.name,b.w-48),b.x+b.w/2,b.y+41);ctx.textAlign="left";ctx.textBaseline="alphabetic";
