...
```bash
  mkdir update
  cd update
  curl -fL -o cangling-update https://github.com/zhangjianshe/cangling-update/releases/download/v0.1.138/cangling-update-linux-amd64
...
5. 「备份数据库」使用数据库容器内的 `pg_dump -Fc` 创建完整逻辑备份，并通过 `pg_dumpall --globals-only` 保存角色和权限；备份历史记录 PostgreSQL/PostGIS 版本、表、视图、序列、文件大小和 SHA-256。
6. 备份历史可恢复整个数据库，或只恢复同一 schema 中选择的一张或多张表的数据。单表恢复会校正关联序列；普通视图只有定义、没有独立数据。覆盖前必须输入数据库名，并自动再备份一次当前数据库。
7. 「自动备份」可设置每日执行时间（默认服务器本地时间 03:00）、保留份数（默认 7 份）以及可选的已挂载存储。错过执行时间会在当天补执行；程序意外重启会把中断任务标记失败并重新调度。自动清理只影响定时备份，不删除手工、恢复前或升级前备份。启用“项目升级前自动备份”后，数据库备份失败会取消导入发布或替换重启。

数据库备份位于 `config/db-backups/<项目ID>/<备份ID>/`，其中包括 `database.dump`、`globals.sql` 和 `manifest.json`。目录权限为 `0700`，备份文件权限为 `0600`。恢复时会先校验 SHA-256；较新 PostgreSQL 的备份不能恢复到较旧版本，PostGIS 备份要求目标容器已经安装 PostGIS。单表恢复不会使用 `CASCADE`，如果未选择的表通过外键引用目标表，将拒绝执行；载入失败时会自动使用恢复前安全备份还原所选表。
...
config/
  ...
  db-backups/<项目ID>/<备份ID>/ # PostgreSQL/PostGIS 逻辑备份、角色和清单
  backups/<项目ID>/<版本ID>/
